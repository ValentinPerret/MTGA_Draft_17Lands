...
from src.game_review.models import CodexGameReview, ParsedGame
...
def build_game_review_payload(game: ParsedGame) -> Dict:
...
class CodexGameReviewer:
    ...
    def review(self, game: ParsedGame, timeout_seconds: float = 120, model: str = "") -> CodexGameReview:
        ...
        prompt = (
            ...
            "strengths as well as mistakes. Set source=codex for every finding. Return only "
        ...
        with tempfile.TemporaryDirectory(prefix="mtga-game-review-") as temp_dir:
            ...
            schema_path.write_text(
                json.dumps(CodexGameReview.model_json_schema(), indent=2),
                encoding="utf-8",
            )
            command = [
                binary,
